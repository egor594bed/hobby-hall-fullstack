...
router.get(
    ...
    async (req: Request, res: Response) => {
    try {
        const goodsArr = await CatalogService.getGoodsFromId(req.query.id as string)
...
router.get(
    ...
    async (req: Request, res: Response) => {
    try {
        const goodsArr = await CatalogService.getGoodsFromSearch(req.query.search as string)
        return res.status(200).json({goodsArr})
    } catch (e) {
        return res.status(500).json({message: 'Что-то пошло не так'})
    }
