...
const BasketForm: FC<IBasketForm> = ({disableButton, setBasketArr, basketArr}) => {
    ...
    function changeActiveOption(e: React.MouseEvent<HTMLDivElement, MouseEvent>){
        ...
        if(target.name === 'delivery') {
            const newActiveDelivery = deliveryArr.find((elem) => elem.id === Number(target.value))
            setActiveDelivery(newActiveDelivery!) 
        }
        else if(target.name === 'payment') {
            const newActivePayment = paymentArr.find((elem) => elem.id === Number(target.value))
    ...
    return (
        <form onSubmit={handleSubmit(postNewOrder)}>
            <select className={cl.MySelect} {...register('delivery', {
            ...
            <div className='basket__delivery-wrapper' style={activeDelivery ? {background: 'none'} : {backgroundImage: `url(${require('../../assets/img/selectImg.png')})`}}>
                {activeDelivery &&
                    <>
                    <img className='basket__delivery-img' src={require(`../../assets/img/delivery/${activeDelivery.imgName}`)}></img>
            ...
            <div className='basket__payment-wrapper' style={activePayment ? {background: 'none'} : {backgroundImage: `url(${require('../../assets/img/selectImg.png')})`}}>
                {activePayment &&
                    <>
                    <img className='basket__payment-img' src={require(`../../assets/img/payment/${activePayment.imgName}`)}></img>
            ...
            <div className='basket__comment'>
                ...
                <div className='basket__comment-wrapper'>
                    ...
                    <textarea className='basket__comment-input' {...register('comment')} placeholder='Оставте свой комментарий'/>
                </div>
            </div>
            <div className='basket__order'>
                ...
                <MyButton style={{fontSize: '20px'}} disabled={disableButton || !isValid}>Оформить заказ</MyButton>
            </div>
        </form>
    )
}
