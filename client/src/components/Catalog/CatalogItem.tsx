...
const CatalogItem: FC<IProduct> = (productData) => {
    const { isFavorite, favoriteHandler} = useFavorite(productData._id, productData.name)
    const {inBasket, basketHandler} = useBasket(productData._id, productData.name)
                ...
                <div className='catalog__item-wrapper'>
                    ...
                    <div className='catalog__item-wrapper-bottom'>
                        ...
                        {productData.quantity < 1
                            ...
                            style={{width: '50%'}}
                            >{(inBasket) ? 'Удалить' : 'В корзину'}</MyButton>
                        }
                    </div>
                </div>
            </div>
