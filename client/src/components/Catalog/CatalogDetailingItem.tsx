...
const CatalogDetailingItem = () => {
  ...
  useEffect(() => {
    async function getProduct() {
      await request(`/api/catalog/getProduct?id=${params.id}`).then((data) =>
  ...
  return (
    <div className="catalog__detail">
      ...
      <div className="catalog__detail-top">
        <img
          className="catalog__detail-top-img"
          src={`/${product.imgSrc}`}
          onError={(event) =>
        ...
        <div className="catalog__detail-top-side">
          ...
          <div className="catalog__detail-top-side-wrapper">
            ...
            {product.quantity < 1 ? (
              ...
              <MyButton onClick={basketHandler} style={{ width: "60%" }}>
                {inBasket ? "Удалить" : "Добавить в корзину"}
              </MyButton>
            )}
          </div>
          <Link to="../../basket">
            <MyButton onClick={noop} style={{ width: "100%" }}>
              Перейти в корзину
            </MyButton>
          </Link>
        </div>
      ...
      <div className="catalog__detail-description">
        <p className="catalog__detail-description-text">
